...
w1=importdata('C:\Users\1\Desktop\项目\温度补偿\温度实验数据\对比测量2\20211128-t.csv');
w2=importdata('C:\Users\1\Desktop\项目\温度补偿\温度实验数据\对比测量2\20211128-m.csv');
w3=importdata('C:\Users\1\Desktop\项目\温度补偿\温度实验数据\对比测量2\20211128-p.csv');

end

temperature(:,1)=w1.data(:,10);
temperature2(:,1)=w1.data(:,12);

buchang = mean(temperature) - mean(temperature2);
temperature2(:,1) = buchang +temperature2(:,1) + 0.05;
...
for i=1:1
% average_tem          = textread('C:/Users/1/Desktop/PSO_HDL/tb/pso_top_tb_input/pso_top_temp_dec.txt','%f') *10;
% average_pre          = textread('C:/Users/1/Desktop/PSO_HDL/tb/pso_top_tb_input/pso_top_pres_dec.txt','%f')*1000;
% average_dis_ch1      = (textread('C:/Users/1/Desktop/PSO_HDL/tb/pso_top_tb_input/pso_top_disp_dec.txt','%f'))'; 
% average_dis_ch2      = (textread('C:/Users/1/Desktop/PSO_HDL/tb/pso_top_tb_input/pso_top_disp_dec.txt','%f'))';  


[average_tem,average_dis_ch1,average_pre] =data_handle(displacement,temperature(:,1),pressure);
[average_tem2,average_dis_ch2,average_pre] =data_handle(displacement2,temperature2(:,1),pressure);

time = 0:10/60/60:(length(average_tem)-1)*1/6/60;
plot(time,average_tem2-average_tem);
hold on ;
plot (time,average_tem)
...
[fitnessgbest_e1,gbest_edpso_ch1]=PSO(average_tem,average_dis_ch1,average_pre,o_length,10,1); %1204
[fitnessgbest_e2,gbest_edpso_ch2]=PSO(average_tem,average_dis_ch2,average_pre,o_length2,10,1); %1204
...
n_edpso_ch1(i)                = gbest_edpso_ch1(1) * average_pre(i) + gbest_edpso_ch1(2) * average_tem(i);
n_edpso_ch2(i)                = gbest_edpso_ch2(1) * average_pre(i) + gbest_edpso_ch2(2) * average_tem(i);
...
compensation_edpso_ch1(i) = (o_length  / 1000 * (n_edpso_ch1(i)-n_edpso_ch1(1)) * 10^9);
compensation_edpso_ch2(i) = (o_length2 / 1000 * (n_edpso_ch2(i)-n_edpso_ch2(1)) * 10^9);
end
...
last_segindex = 1;
seg_num = 1;
segindex =mini_width+1;
seg_index(1) = 1;
while segindex<=length(average_tem)
if(abs( average_tem(segindex) - average_tem(last_segindex) )>=diff_value)
    seg_num = seg_num+1;
    seg_index(seg_num) = segindex;
    last_segindex = segindex;
    segindex = segindex+mini_width;
else
    segindex = segindex+1;
end 
end
seg_num = seg_num+1;
seg_index(seg_num) = length(average_tem);
for i = 1 : seg_num-1
    [fitnessgbest_e_burst_ch1{i},gbest_e_burst_ch1{i}] =PSO([average_tem(seg_index(i) : seg_index(i+1)-1)],[average_dis_ch1(seg_index(i) : seg_index(i+1)-1)],[average_pre(seg_index(i) : seg_index(i+1)-1)],o_length,10,0);
    [fitnessgbest_e_burst_ch2{i},gbest_e_burst_ch2{i}] =PSO([average_tem(seg_index(i) : seg_index(i+1)-1)],[average_dis_ch2(seg_index(i) : seg_index(i+1)-1)],[average_pre(seg_index(i) : seg_index(i+1)-1)],o_length2,10,0);
    if i == seg_num-1
        kouchuzhi = 0;
    else
        kouchuzhi = 1;
    end
    for t = seg_index(i) :seg_index(i+1)-kouchuzhi
        index = t;
        ...
        compensation_edpso_burst_ch1(index) = (o_length  / 1000 * (n_edpso_burst_ch1(index)-n_edpso_burst_ch1(seg_index(i))) * 10^9);
        compensation_edpso_burst_ch2(index) = (o_length2 / 1000 * (n_edpso_burst_ch2(index)-n_edpso_burst_ch2(seg_index(i))) * 10^9);
    end
end
%%将每一段训练的结果减去上一段的累计误差，这边采用一个1*10的固定窗口对上一段的训练结果求平均从而计算累计误差
for i = 1:seg_num-1
    if(i~=1)
        burst_error_ch1 = mean(compensation_edpso_burst_ch1(seg_index(i) : seg_index(i)+10)) - mean(compensation_edpso_burst_ch1(seg_index(i)-10 : seg_index(i)-1));
        compensation_edpso_burst_ch1(seg_index(i) : seg_index(i+1)-1) = compensation_edpso_burst_ch1(seg_index(i) : seg_index(i+1)-1) - burst_error_ch1;
        burst_error_ch2 = mean(compensation_edpso_burst_ch2(seg_index(i) : seg_index(i)+10)) - mean(compensation_edpso_burst_ch2(seg_index(i)-10 : seg_index(i)-1));
        compensation_edpso_burst_ch2(seg_index(i) : seg_index(i+1)-1) = compensation_edpso_burst_ch2(seg_index(i) : seg_index(i+1)-1) - burst_error_ch2;       
    end
    if(i == seg_num-1)
        compensation_edpso_burst_ch1(seg_index(i+1)) = compensation_edpso_burst_ch1(seg_index(i+1)) - burst_error_ch1;
        compensation_edpso_burst_ch2(seg_index(i+1)) = compensation_edpso_burst_ch2(seg_index(i+1)) - burst_error_ch2;
    end
...
%         set(h2,'FontName','Times New Roman','FontSize',8,'FontWeight','normal')
     ...
     hfigure=figure('numbertitle','off','name','两个通道补偿效果对比');
        ...
        line4=plot(time(1),result_edpso_burst_ch2_filter(1),'-p','Color',[0.19 0.6 0.18]);
        hold on;
        plot(time(ceil(market_index)),result_edpso_burst_ch2_filter(ceil(market_index)),'p','Color',[0.19 0.6 0.18]);
        hold on;
        plot(time,result_edpso_burst_ch2_filter,'LineWidth',1.5,'Color',[0.19 0.6 0.18]);
        
        pos1=axis;
        xlabel('time/h','FontName','Times New Roman','FontSize',8);
        ylabel('displacement/nm','FontName','Times New Roman','FontSize',8);
        h1=legend([line1 line2 line3 line4],'Raw data','Unoptimized Edlen','Optimized Edlen','Segmented edlen'); 
        xlim([time(1) time(length(time))]);
        title('(a) 45mm','FontSize',21,'position',[0.06 20.69  0]);
        set(gca,'FontSize',12) ;
        ...
        line24=plot(time(1),result_edpso_burst_ch1_filter(1),'-p','Color',[0.19 0.6 0.18]);
        hold on;
        plot(time(ceil(market_index)),result_edpso_burst_ch1_filter(ceil(market_index)),'p','Color',[0.19 0.6 0.18]);
        hold on;
        plot(time,result_edpso_burst_ch1_filter,'LineWidth',1.5,'Color',[0.19 0.6 0.18]);
        
        pos2=axis;
        xlabel('time/h','FontName','Times New Roman','FontSize',8');
        ylabel('displacement/nm','FontName','Times New Roman','FontSize',8)
        h2=legend([line21 line22 line23 line24],'Raw data','Unoptimized Edlen','Optimized Edlen','Segmented edlen'); 
%         set(h2,'FontName','Times New Roman','FontSize',8,'FontWeight','normal')
        xlim([time(1) time(length(time))]);
        title('(b) 90mm','FontSize',21,'position',[0.061 51.626  0]);
        set(gca,'FontSize',12) ;
